...
def post_detail(request, pk):
    ...
    chat_room, created = ChatRoom.objects.get_or_create(name=post.id)
...
def send_message(request):
    if request.method == 'POST':
        ...
        # Retrieve the ChatRoom instance based on the room_id
        room = get_object_or_404(ChatRoom, id=room_id)

        # Create a new Message object with the correct ChatRoom instance
...
def get_messages(request, post_id):
    if request.method == 'GET':
        ...
        messages = Message.objects.filter(post_id=post_id).order_by('date')
